        ...
        private void SendRandomPassword(string email, string code)
        {
            ...
            { 
                if (Session["gap"] == "0") {
                    MySqlConnection connection = new MySqlConnection(connectionString);
                    connection.Open();
                    string query = "SELECT cEmail FROM CarerLogin WHERE cEmail = @Email";
                    MySqlCommand command = new MySqlCommand(query, connection);
                    command.Parameters.AddWithValue("@Email", forgetPass_Text.Text.Trim()); // 确保去除多余空格
                    MySqlDataReader reader = command.ExecuteReader();
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                ...
                else if (Session["gap"] == "1")
                {
                    ...
                    MySqlCommand command = new MySqlCommand(query, connection);
                    command.Parameters.AddWithValue("@homeEmail", forgetPass_Text.Text);
                    command.Parameters.AddWithValue("@homePassword", code);
                    command.ExecuteNonQuery();
                    connection.Close();
                }
            }
